...
initializeCleaner();
var trashQuantity = Math.floor(Math.random() * (10 - 1 + 1) + 1);
var attempQuantity = Math.floor(Math.random() * (3 - 2 + 1) + 2);
document.getElementById("trashQuantityLabel").innerText = "Cantidad de basuras: " + trashQuantity;
document.getElementById("attempsQuantityLabel").innerText = "Intentos para aspirar: " + attempQuantity;
...
function initializeCleaner(){
    ...
    if(side % 2 == 0){
        document.getElementById('aspiradora').style.left = 370;
    } else {
        document.getElementById('aspiradora').style.left = 870;
    }
}
...
async function fillInBoard(x) {
    ...
    while (valori <= x) {
        while (trash == 8) {
            console.log("Espere 5 seg mientras la aspiradora limpia...");
            await sleep(5000);
        }
...
function changeSector() {
    console.log("Cambiando de sector")
    if (aspiradora[1] == 1) {
        aspiradora[1] = 0;
        return
    }
...
function start() {
    ...
    fillInBoard(parseInt(trashQuantity));
    findAllTrash(parseInt(attempQuantity));
    document.getElementById("intentosAspirar").innerHTML=0;
    attemptCounter=0;
...
async function findAllTrash(attempt) {
    ...
    if (trash == 0) {
        console.log("No se encontró basura, cambiando de sector...");
        if(aspiradora[1] == 1){
            moverAspiradora(0, 1);
        } else if(aspiradora[1] == 0){
            moverAspiradora(1, 0);
        }
        await sleep(7000);
        sector = null;
        ...
        document.getElementById("intentosAspirar").innerHTML=attemptCounter;
    }
